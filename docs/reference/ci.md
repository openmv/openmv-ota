...
|---|---|---|
| **full** (OTA-capable) | N6, AE3, 4P, PT, RT1060, Portenta, Giga, Nicla | `project new --ota`; build firmware + romfs + factory-romfs; `inspect` + `verify` the OTA bundle (as a `.zip` and as loose `romfs.img`/`trailer.bin`) **and the provisioning image** (both slots, A + B); a corrupted body **and** a corrupted slot must **fail** verify. A multi-core board (AE3) also builds + checks its plain `coprocessor-romfs.img`. N6/AE3/RT1060 build with **no** `--ca` — the firmware freezes the full public bundle for recovery, and compiling it is the link-fit proof of that default; the 1792 KB boards (4P, PT, Portenta, Giga, Nicla) must **refuse** `project new --ota` without `--ca` (the bundle overflows their FLASH_TEXT) and are built with a generated root. |
| **classic** (one-slot partition → single-image OTA) | OPENMV2 / 3 / 4 | `project new --ota` must refuse **without** `--ca` (the public bundle doesn't fit the slot) and **succeed with** your own root (single-image mode); the supplied root — not the ~186 KB bundle — must be the frozen trust store; `build romfs` must name the *single-image* slot bound either way — the stripped installer fits an M7-classic's 248 K slot (successful build) but not an M4/H7-classic's 120 K one (clean refusal); the bound's wording is the mode-aware proof. The plain path still holds: `project new`; build firmware + single-image romfs; `factory-romfs` must fail cleanly (*needs an OTA project*). OPENMV4 skips the OTA firmware *compile* (FLASH_TEXT overflow on upstream master; the fix is fork-maintained — the HIL legs pin that branch); the M4/M7 build it. |
