...
| **classic** (one-slot partition → single-image OTA) | OPENMV2 / 3 / 4 | `project new --ota` must refuse **without** `--ca` (the public bundle doesn't fit the slot) and **succeed with** your own root (single-image mode); the supplied root — not the ~186 KB bundle — must be the frozen trust store; `build romfs` must name the *single-image* slot bound either way — the stripped installer fits an M7-classic's 240 K slot (successful build) but not an M4/H7-classic's 112 K one (clean refusal); the bound's wording is the mode-aware proof. The plain path still holds: `project new`; build firmware + single-image romfs; `factory-romfs` must fail cleanly (*needs an OTA project*). OPENMV4 skips the OTA firmware *compile* (a known FLASH_TEXT overflow, being trimmed); the M4/M7 build it. |
