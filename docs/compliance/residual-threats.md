...
| **Vulnerability-database coverage** | Accepted (bounded) | The daily SBOM scans match firmware C libraries against OSV by name and version, and OSV's coverage of C projects is thinner than its package-ecosystem coverage — a clean scan means "no known issues in one good database", not a guarantee. Bounds: the scan is one input beside coordinated disclosure and the vendor's own review; the interface takes a component list, so a second source (NVD-based matching) can be added without redesign. |
| **Cross-product installs on platform hardware** (`product_id = 0`) | Accepted (opt-in) | A camera deliberately built with `product_id = 0` — a platform's stock unit, meant to move between that account's products — has the cross-flash guard off, so any image the account signs is a candidate for it. Bounds: only that account's images verify (one key set, baked into the firmware); the server offers only within the camera's bound account; and every such image carries the account's publish counter, which the camera enforces as its anti-rollback floor — so an older image, whichever product it came from, cannot come back. An ordinary camera (nonzero `product_id`) is untouched by this. |
