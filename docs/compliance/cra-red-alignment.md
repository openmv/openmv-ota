...
| Requirement | How this stack supports it |
|---|---|
| (1) Identify and document components; SBOM in a machine-readable format | **Shipped:** `build sbom` renders CycloneDX 1.5 from the lock (firmware commit, every submodule commit at its release version, toolchain versions, the app's own repo commit and license) — deterministic per lock, uploaded with every release and served back byte-exact |
| (2) Address and remediate vulnerabilities without delay; security updates separable from feature updates | **Shipped:** the server scans the SBOM of every release the fleet still runs against OSV.dev daily and on publish (`client advisories`), records findings in an audited history that clears only when a rescan stops reporting them or the fleet moves past the release — so "without delay" starts at automatic detection, not at someone reading a mailing list. Remediation is OTA delivery with staged rollouts and per-device/cohort pins; a security-only release is ordinary release discipline |
