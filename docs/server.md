...
| browser UI origins | `OPENMV_OTA_CORS_ALLOW_ORIGINS` | comma-separated origins allowed to call this API **cross-origin**, e.g. `https://cloud.openmv.io`. Empty by default = no CORS headers at all. Needed only when a UI is served from a *different* origin than this app; a same-origin UI, or one that proxies through its own backend, leaves it unset. No `*` is accepted -- name the origins |
