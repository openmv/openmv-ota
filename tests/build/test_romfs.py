...
# --- directory bases are collections, not claims ---------------------------------------------

def test_dir_sourced_base_not_older_is_skipped_not_fatal(make_project, capsys):
    """`client release bases --fleet` legitimately downloads the CURRENT release too, so a
    directory-sourced base at/above the version being built is SKIPPED with a note. An
    explicitly named file stays an error -- naming it is a claim."""
    ...
    [r1] = build_mod.build_ota_romfs(root, firmware=repo, delta_from=factory,
                                     compile_py=False, convert_models=False)
    bases = root / "build" / "bases"
    bases.mkdir()
    _shutil.copy2(r1.image, bases / "OPENMV_N6-base-1.1.0.img.gz")   # SAME version as the build

    [r2] = build_mod.build_ota_romfs(root, firmware=repo, delta_from=bases,
                                     compile_py=False, convert_models=False,
                                     allow_republish=True)
    err = capsys.readouterr().err
    assert r2.deltas == [] and "skipping base" in err                # noted, never fatal

    with pytest.raises(BuildError, match="not older than this release"):
        build_mod.build_ota_romfs(root, firmware=repo,
