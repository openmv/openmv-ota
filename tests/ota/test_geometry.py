...
def test_slot_overhead_is_two_blocks():
    assert geometry.slot_overhead(4096) == 8192
    assert geometry.slot_overhead(16) == 8192   # floored block -> 2 x 4 KiB
    assert geometry.slot_overhead(131072) == 262144


def test_capable_nor_partition():
    assert geometry.is_ota_capable(0x1800000, 4096)         # 24 MiB NOR
    assert geometry.body_capacity(0x1800000, 4096) == 0xC00000 - 8192
...
def test_mram_partition_capable_with_floor():
    ...
    assert geometry.body_capacity(0x100000, 16) == 0x80000 - 8192
