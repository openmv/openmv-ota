...
a fully-written entry counts. The sector holds hundreds of entries before it must be erased
and compacted to the current floor.
"""
...
def append_offset(sector) -> int | None:
    ...
    (the caller must then erase + re-seed it with the current floor)."""
    for i in range(0, len(sector) - ENTRY_SIZE + 1, ENTRY_SIZE):
